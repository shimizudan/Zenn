...
- $a \leq 5$ かつ $b$ が偶数のとき: $a$ を返す（1〜5）
- $a \leq 5$ かつ $b$ が奇数のとき: $a + 5$ を返す（6〜10）
- $a = 6$ のとき: やり直す
...
function saikoro10()
    ...
    elseif a <= 5 && isodd(b)
        ...
        saikoro10()  # a=6が出たらやり直し
    end
end

# 100万回試行
result = [saikoro10() for _ = 1:10^6] |> countmap
```
...
この方法では、1回目が1〜5（確率 $\frac{5}{6}$）かつ2回目が偶数/奇数（各確率 $\frac{1}{2}$）により、各数字が確率 $\frac{5}{6} \times \frac{1}{2} = \frac{5}{12}$ で生成されます。1回目が6の場合（確率 $\frac{1}{6}$）はやり直すため、最終的に各数字が $\frac{1}{10}$ の確率で均等に生成されます。

:::message
**謝辞**: この方法について、[@gavangavan](https://x.com/gavangavan)さんより誤りをご指摘いただき、より適切な解法に修正することができました。ありがとうございました。
:::
...
- 2回のサイコロの出目の組み合わせ（値と偶奇）を利用する方法も有効
